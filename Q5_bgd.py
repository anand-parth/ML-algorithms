...
for _ in range(iterations):
	...
	val,val2,val3 = [0,0,0]
	for i in range(m):
		val = val + ((h[i]-y[i])*(X[i][0])) 
		val2 = val2 + ((h[i]-y[i])*(X[i][1]))
		val3 = val3 + ((h[i]-y[i])*(X[i][2]))
	w[0] = w[0]*(1-lamda*alpha*np.sign(w[0])/2) - ((alpha/m)*val)
	w[1] = w[1]*(1-lamda*alpha*np.sign(w[1])/2) - ((alpha/m)*val2)
	w[2] = w[2]*(1-lamda*alpha*np.sign(w[2])/2) - ((alpha/m)*val3)
